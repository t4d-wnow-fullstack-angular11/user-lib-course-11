(function (global, factory) {
    ...
    var setCurrentUser = store.createAction('[UserLib] Set Current User', store.props());
    var clearCurrentUser = store.createAction('[UserLib] Clear Current User');
    var loginUser = store.createAction('[UserLib] Login User', store.props());
    var logoutUser = store.createAction('[UserLib] Logout User');
    ...
    var currentUserReducer = store.createReducer(initialState, store.on(setCurrentUser, ɵ0), store.on(clearCurrentUser, ɵ1));

    var CurrentUserEffects = /** @class */ (function () {
        function CurrentUserEffects(actions$, usersSvc) {
            var _this = this;
            this.actions$ = actions$;
            this.usersSvc = usersSvc;
            this.logoutUser$ = effects.createEffect(function () { return _this.actions$.pipe(effects.ofType('[UserLib] Logout User'), operators.mergeMap(function () { return rxjs.of(sharedLib.clearErrorMessage(), clearCurrentUser()); })); });
            this.loginUser$ = effects.createEffect(function () { return _this.actions$.pipe(effects.ofType('[UserLib] Login User'), operators.exhaustMap(function (action) { return _this.usersSvc.loginEmployee(action.username, action.password).pipe(operators.mergeMap(function () {
                return rxjs.of(setCurrentUser({ currentUser: _this.usersSvc.getCurrentUser() }), sharedLib.clearErrorMessage());
            }), operators.catchError(function (err) {
                if (err.status === 404) {
                    return rxjs.of(sharedLib.setErrorMessage({ errorMessage: 'Username and password not found.' }), clearCurrentUser());
                }
                else {
                    return rxjs.of(sharedLib.setErrorMessage({ errorMessage: 'Unknown login error.' }), clearCurrentUser());
                }
            })); })); });
        }
    ...
    CurrentUserEffects.ctorParameters = function () { return [
        { type: effects.Actions },
        { type: UsersService }
    ]; };

    var StoreFeatureModule = store.StoreModule.forFeature('user', { currentUser: currentUserReducer });
    var EffectsFeatureModule = effects.EffectsModule.forFeature([CurrentUserEffects]);
    ...
    var ɵ0$1 = function (state) { return state.user.currentUser; }, ɵ1$1 = function (currentUser) { return currentUser; };
    var selectCurrentUser = store.createSelector(ɵ0$1, ɵ1$1);
    ...
    exports.ɵ0 = ɵ0$1;
    exports.ɵ1 = ɵ1$1;
    exports.ɵa = initialState;
    exports.ɵb = currentUserReducer;
    exports.ɵc = CurrentUserEffects;
